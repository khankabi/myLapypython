...
cls refers to class itself however no need to use only cls any other literal is also used
example:
@classmethod
def clsmethod(sonu,num)
    sonu.num=num
sonu as a pointer to class
...
Conclusion : class method is used to access class variable just like normal variable
...
class Employee:
  ...
  @classmethod
  def changeCompany(cls, newCompany): #overwrite kardega
    cls.company = newCompany
